...
buttons.addEventListener('click', function (event) {
  ...
  if (target.matches('button')) {
    ...
    if (action === 'number') {
      ...
      // 아래 코드가 작동됩니다.
      console.log('숫자 ' + buttonContent + ' 버튼5');
      // 첫 번째 칸에 입력된 내용이 있는지, 없는지 구분해
      // 첫 번째 칸에 입력된 내용이 0(기본값)이 아니라면, 이미 숫자가 입력된 상태로 볼 수 있습니다.
      // 첫 번째 숫자가 0이 아닌 경우, 버튼을 클릭하면 두 번째 칸에 버튼에 적혀있는 숫자를 입력
    ...
    if (action === 'calculate') {
      console.log('계산 버튼');
      // 첫 번째 숫자, 연산자, 두 번째 숫자를 확정
      // 위 세 가지를 함수 calculate에 전달하고, 돌려받은 결괏값이 마지막 칸에 입력
